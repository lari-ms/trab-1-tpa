    ...
    private static void gerarArquivo() {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(NOME_ARQUIVO))) {
            ...
            for (int i = 1; i <= NUM_REGISTROS; i++) {
                ...
                float salario = random.nextFloat() * 10000; // Gera uma nota entre 0 e 10
                long cpf = 10000000000L + (long)(random.nextDouble() * 90000000000L);
                writer.write(nome + ";" + String.format("%.2f", salario).replace(',', '.') + ";" + String.valueOf(cpf)+ "\n");
                if (i % 1_000_000 == 0) {
